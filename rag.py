...
docs = []
for pdf_path in select_all_docs():
    docs.extend(extract(pdf_path))
...
llm = Llama(
    model_path=select_model(2), #'/models/Mistral-7B-Instruct-v0.3.Q8_0/Mistral-7B-Instruct-v0.3.Q8_0.gguf',
    n_gpu_layers=2,
    temperature=0.1,
    n_threads=20,
    n_ctx = 8000,
...
def ask(prompt):
    print("Generating...")
    return llm(prompt=prompt, max_tokens=500, temperature=0.7, top_p = 0.95)['choices'][0]['text']
